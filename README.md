...
格式化与构建入口：`cargo fmt --all`、[Cargo.toml](Cargo.toml)、[apps/desktop/package.json](apps/desktop/package.json)、[迁移说明](Docs/rust-migration.md)。当前仓库已包含 Rust core、script、input、hotkey、capture、vision、task、CLI 与 Tauri/Svelte desktop scaffold；原 C#/.NET 实现保留为逐步迁移参考，待 Rust parity 验证后移除。
