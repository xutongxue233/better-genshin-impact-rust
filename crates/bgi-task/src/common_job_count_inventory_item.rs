...
pub fn plan_count_inventory_item(
    ...
    Ok(CountInventoryItemExecutionPlan {
        ...
        result_contract,
        steps,
        notes: "Inventory open/tab selection, grid constants, icon crop, prototype matching, count OCR, weapon-ore prescroll, and single/multiple result contracts are modeled and executable as a Rust state machine through injectable hooks; desktop live routing now performs shared game-window, BitBlt, and capture-size preflight, executes OpenInventory through SendInput with post-open template state probes, confirms expired-item prompts, clicks inventory tabs, verifies GridIcon classifier asset availability, parses GridIcon prototype assets, dispatches the weapon-ore scrollbar-bottom hold through capture-relative input, applies the legacy ordinary-inventory first-page de-highlight double-click/two-frame column stitching before visible-page grid enumeration, crops normalized icons from that stitched visible-page cell cache, and gates final not-found results on explicit scan completion, while full GridScroller scroll-to-bottom semantics, post-first-page anti-recycling clicks, ONNX feature extraction, Paddle OCR, and script return object execution remain pending.".to_string(),
    })
}
