...
pub fn plan_count_inventory_item(
    ...
    Ok(CountInventoryItemExecutionPlan {
        ...
        result_contract,
        steps,
        notes: "Inventory open/tab selection, grid constants, icon crop, ONNX prototype matching, count OCR, weapon-ore prescroll, and single/multiple result contracts are modeled and executable as a Rust state machine through injectable hooks; desktop live routing now performs shared game-window, BitBlt, and capture-size preflight, executes OpenInventory through SendInput with post-open template state probes, confirms expired-item prompts, clicks inventory tabs, verifies GridIcon classifier asset availability, dispatches the weapon-ore scrollbar-bottom hold through capture-relative input, can crop enumerated grid cells into normalized icon images from the current frame, and reports the first active missing inventory adapter from the plan, while OpenCV grid enumeration with GridScroller-style scroll-to-bottom, ONNX/prototype classifier loading and inference, Paddle OCR, and script return object execution remain pending.".to_string(),
    })
}
