...
pub fn plan_go_to_crafting_bench(
    ...
    Ok(GoToCraftingBenchExecutionPlan {
        ...
        resin_craft_rule,
        steps,
        notes: "Legacy crafting-bench pathing, dialogue interaction, condensed-resin recognition, and crafting flow are represented and executable through injectable pathing/OCR/craft hooks; bundled PathExecutor JSON can be validated through the shared movement-contract preflight, and the desktop live route now has a capture/OCR/send-input adapter for the crafting-bench F interaction retry. Desktop live movement, talk-option selection, one-dragon config loading, OCR, and button click execution remain pending.".to_string(),
    })
}
