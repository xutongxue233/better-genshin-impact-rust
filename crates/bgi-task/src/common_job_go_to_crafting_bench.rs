...
pub fn plan_go_to_crafting_bench(
    ...
    Ok(GoToCraftingBenchExecutionPlan {
        ...
        resin_craft_rule,
        steps,
        notes: "Legacy crafting-bench pathing, dialogue interaction, condensed-resin recognition, and crafting flow are represented and executable through injectable pathing/OCR/craft hooks; bundled PathExecutor JSON can be validated through the shared movement-contract preflight, and the desktop live route now has capture/OCR/send-input adapters for the crafting-bench F interaction retry plus legacy-style talk-option draining with crafting-page and main-UI stop checks. Desktop live movement, one-dragon config loading, resin OCR, and condensed-resin button click execution remain pending.".to_string(),
    })
}
