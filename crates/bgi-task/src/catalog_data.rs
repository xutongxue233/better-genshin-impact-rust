...
static TASK_CATALOG: &[TaskCatalogEntry] = &[
    ...
    TaskCatalogEntry {
        ...
        requires_main_ui_wait: Some(true),
        port_state: TaskPortState::RuntimeScaffolded,
        notes: "Rust models GetGridIcons config, gridName string/numeric compatibility, inventory/manual-open policy, GridScreen and ArtifactSetFilter grid constants, item-name OCR crop, ArtifactSetFilter flower-name double-OCR selection and icon-crop planning, optional star suffix color-contour rule, timestamped output directory, duplicate filename skip, PNG save behavior, related GridIconsAccuracyTest model metadata, independent-task invocation expansion, and an injectable executor for open/scan/save/cleanup reports; the desktop independent live route now performs config parsing, capture preflight, output-directory creation, no-scan execution, and explicit next-adapter errors, while desktop live full capture, OpenCV enumeration, Paddle OCR, input dispatch, overlay cleanup, and ONNX adapters remain pending.",
    },
    TaskCatalogEntry {
        key: "Shell",
