...
static TASK_CATALOG: &[TaskCatalogEntry] = &[
    ...
    TaskCatalogEntry {
        ...
        requires_main_ui_wait: Some(true),
        port_state: TaskPortState::RuntimeScaffolded,
        notes: "Rust models GetGridIcons config, gridName string/numeric compatibility, inventory/manual-open policy, GridScreen and ArtifactSetFilter grid constants, item-name OCR crop, ArtifactSetFilter flower-name double-OCR selection and icon-crop planning, optional star suffix color-contour rule, timestamped output directory, duplicate filename skip, PNG save behavior, related GridIconsAccuracyTest model metadata, independent-task invocation expansion, and an injectable executor for open/scan/save/cleanup reports; the runtime enumeration now carries scan_complete, and the desktop independent live route performs config parsing, capture preflight, output-directory creation, no-scan execution, and routes ordinary inventory pages through the shared current-visible-page inventory enumeration/crop helper, ordinary item-name WinRT OCR, optional star suffix color-component detection, and legacy-compatible pre-click full-cell PNG encoding with scan_complete=false protection before explicit GridScroller/ONNX adapter gaps, while full GridScroller page scrolling, Paddle OCR parity, artifact OCR, ONNX-backed GridIcon classification, exact OpenCV parity, special-grid input, and overlay cleanup remain pending.",
    },
    TaskCatalogEntry {
        key: "Shell",
