...
static TASK_CATALOG: &[TaskCatalogEntry] = &[
    ...
    TaskCatalogEntry {
        ...
        requires_main_ui_wait: None,
        port_state: TaskPortState::RuntimeScaffolded,
        notes: "Rust now preserves and executes the inventory tab, grid enumeration, icon classification, count OCR, weapon-ore prescroll, and single/multiple result-contract state machine through injectable hooks and invocation live bridge; desktop common-job live routing, AutoEatFood inventory execution, and GetGridIcons ordinary inventory pages share game-window, BitBlt, capture-size, inventory open/prompt/tab handling, current-visible-page enumeration, and grid-icon crop helpers instead of collapsing inventory open, prompt, tab, GridIcon, grid, input, ONNX, OCR, and click gaps into one unsupported error; GetGridIcons can encode cached pre-click visible-page full-cell crops as PNG and marks that shared visible-page route as scan_complete=false until full GridScroller is wired, while complete GridScroller scan semantics and script return integration remain pending.",
    },
    TaskCatalogEntry {
        key: "Teleport",
