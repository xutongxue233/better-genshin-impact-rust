...
static TASK_CATALOG: &[TaskCatalogEntry] = &[
    ...
    TaskCatalogEntry {
        ...
        requires_main_ui_wait: None,
        port_state: TaskPortState::RuntimeScaffolded,
        notes: "Rust now preserves and executes the Adventurers' Guild party switch, encounter reward, navigation, dialogue, and expedition state machine through injectable hooks and invocation live bridge; the desktop common-job route now shares the BitBlt capture backend, game-window metrics, frame-source, and capture-size guard used by other live common jobs, then runs condition-aware preflight that skips statically inactive party/encounter branches, recognizes existing nested common-job bridges, validates the bundled PathExecutor JSON through the shared movement-contract preflight, and rejects first at the Catherine interaction adapter before talk-option drain adapters are wired.",
    },
    TaskCatalogEntry {
